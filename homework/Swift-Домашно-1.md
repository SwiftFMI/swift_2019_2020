...
1. Да се напише функция, която отпечатва всички достижими полета за определен брой ходове от начална позиция  извърешние с шахматна фигура - кон, без повторение от зададено начално поле. Шахматната дъска има размер 8 х 8.
Да се използва нотацията в шаха в матрица. Т.е. редовете са номерирани от `1` до `8`, а колоните от  `a` до `h`

Пример:

Ако коня започва от поле `а1` и трябва да определим достижимите полета с един ход, тогава получваме списъка от `a1`,  `b3` и  `c2`. (наредбата няма значение)
